...
Once we have extracted the binary image, we will extract the interest points, just searching those pixels coordinates in which its intensity is different from zero (i.e. white pixels).  This results in 11109 points to process; a number which will give us enough information to process and obtain a nice reconstruction. 
...
Additionally, a video around the reconstruction is found clicking on [this link](enlace). 
